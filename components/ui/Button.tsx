...
const variantStyles: Record<ButtonVariant, string> = {
  ...
  secondary:
    "border border-[var(--color-border)] bg-white text-slate-900 hover:border-[var(--color-border-strong)] hover:bg-slate-50 focus-visible:outline-[var(--color-accent)]",
  ghost:
  ...
  ({ className, variant = "primary", size = "md", loading = false, children, disabled, ...props }, ref) => {
    return (
      <button
        ref={ref}
        className={cn(
          "inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl font-medium transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-60",
